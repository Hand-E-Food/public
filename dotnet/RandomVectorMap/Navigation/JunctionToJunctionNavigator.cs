...
public partial class JunctionToJunctionNavigator : Navigator<JunctionToJunctionNavigator.Route>
{
    ...
    public JunctionToJunctionNavigator(IEnumerable<Junction> origins, IEnumerable<Junction> destinations)
    {
        ...
        foreach (var origin in origins)
        {
            ...
            Route route = new(origin, remainder);
            AddCandidate(route);
        }
    }
    ...
    /// </summary>
    public IEnumerable<Junction> Destinations { get; }
    ...
    /// </summary>
    public IEnumerable<Junction> Origins { get; }
    ...
    protected override Route ExtendRoute(Route route, Road road)
    {
        ...
        Route newRoute = new(route.Junctions.First(), route, road, remainder);
        return newRoute;
    }
    ...
    private double GetRemainingDistance(Junction routeEnd) => Destinations.Min(destination => new Vector(routeEnd, destination).Length);
    ...
    protected override bool IsSolution(Route route) => route.Remainder == 0;
    ...
    protected override bool IsViable(Route route)
    {
        return base.IsViable(route)
            && (BestRoute is null || route.Score <= BestRoute.Length)
            && route.Score <= Maximum
